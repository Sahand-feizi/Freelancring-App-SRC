...
function Stats() {
  ...
  return (
    <div className='grid grid-cols-1 grid-rows-3 md:grid-cols-2 md:grid-rows-2 lg:grid-cols-3 lg:grid-rows-1 gap-2'>
      <Stat 
        icon={<HiOutlineViewGrid className="w-10 md:w-16 h-10 md:h-16" />}
        title='پروژه ها'
        value={numOfProjects}
        color='yellow'
      />
      <Stat 
        icon={<HiCurrencyDollar className="w-10 md:w-16 h-10 md:h-16" />}
        title='پروژه های واگذار شده'
        value={numOfAccussProjects}
        color='green'
      />
      <Stat 
        icon={<HiCollection className="w-10 md:w-16 h-10 md:h-16" />}
        title='درخواست ها'
